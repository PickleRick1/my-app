...
export const getMyProfile = () => { // thunkCrеator который делает связанные мелкие диспатчи и отсылает в дал запрос
	return (dispatch) => {
		return authAPI.getMyProfile() // делает аякс запрос за моим айди, логином и мейлом посредством куки с апишки сервера
			.then(data => {
...
export const login = (email, password, rememberMe) => { // thunkCrеator который делает связанные мелкие диспатчи и отсылает в дал запрос
	return (dispatch) => {
		authAPI.login(email, password, rememberMe) // делает аякс запрос за моим айди, логином и мейлом посредством куки с апишки сервера
			.then(data => {
...
export const logout = () => { // thunkCrеator который делает связанные мелкие диспатчи и отсылает в дал запрос
	return (dispatch) => {
		authAPI.logout() // делает аякс запрос за моим айди, логином и мейлом посредством куки с апишки сервера
			.then(data => {
				if (data.resultCode === 0) {
